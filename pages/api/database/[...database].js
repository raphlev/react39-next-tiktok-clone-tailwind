...
let main = async () => {
  let url = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@${process.env.DB_HOSTNAME}?retryWrites=true&w=majority`;
  if (!process.env.DB_USER || !process.env.DB_PASSWORD || !process.env.DB_HOSTNAME) {
    console.log("Check Database environment variable");
    return false;
  } else {
